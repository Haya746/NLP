...
- Notably, POS-aware lemmatization wasn't uniformly reliable — "outdid" stayed unresolved when tagged in isolation (Step 1's four-word list), but was correctly lemmatized to "outdo" once tagged within its full sentence context (Step 3). This shows that lemmatization accuracy depends not just on whether POS tagging is used, but on whether the POS tagger has enough surrounding context to tag correctly in the first place — an isolated word list gives it nothing to work with
